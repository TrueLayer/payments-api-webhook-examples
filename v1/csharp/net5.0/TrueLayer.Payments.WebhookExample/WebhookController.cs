...
namespace TrueLayer.Payments.WebhookExample
{
    ...
    public sealed class WebhookController : ControllerBase
    {
        ...
        public async Task<StatusCodeResult> OnWebhook(
            ...
            [FromHeader(Name = "X-TL-Webhook-Timestamp")]
            string timestamp)
        {
            var payload = GetPayload(webhookBody, timestamp);
        ...
        private sealed class SignaturePayload
        {
            [JsonProperty("Content-Type", Order = 1)]
            public string ContentType { get; set; } = null!;

            [JsonProperty("X-TL-Webhook-Timestamp", Order = 2)]
            public string XTLWebhookTimestamp { get; set; } = null!;
